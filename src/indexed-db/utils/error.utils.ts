  ...
  if (error instanceof Error) {
    ...
    switch (errorName) {
      ...
      case 'ConstraintError':
        throw new Error(`${contextMessage}데이터 제약 조건 위반: ${error.message}`);

      case 'QuotaExceededError':
        throw new Error(`${contextMessage}저장 공간이 부족합니다.`);

      case 'VersionError':
        throw new Error(`${contextMessage}데이터베이스 버전 오류가 발생했습니다.`);

      case 'AbortError':
        throw new Error(`${contextMessage}작업이 중단되었습니다.`);

      case 'DataError':
        throw new Error(`${contextMessage}잘못된 데이터 형식입니다: ${error.message}`);

      case 'InvalidStateError':
        throw new Error(`${contextMessage}데이터베이스가 올바른 상태가 아닙니다.`);

      case 'TimeoutError':
        throw new Error(`${contextMessage}작업 시간이 초과되었습니다.`);
